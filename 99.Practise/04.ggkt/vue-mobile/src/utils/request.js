...
const service = axios.create({
    // baseURL: 'http://ggkt.vipgz1.91tunnel.com', // 前端页面api 的 base_url
    baseURL: 'http://ggktback.sk370.top/',
...
service.interceptors.request.use(config => {
        ...
        if (token != '') {
            //把token值放到header里面，之所以放到header是因为localstorage为本地存储，不支持跨域
...
service.interceptors.response.use(response => {
        if (response.data.code == 208) {
            ...
            window.location = 'http://ggktback.sk370.top/api/user/wechat/authorize?returnUrl=' + url
        } else {
