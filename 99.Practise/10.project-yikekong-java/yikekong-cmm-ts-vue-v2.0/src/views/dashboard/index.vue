<template>
  <div class="dashboard-container" @click.stop="() =>  boardShow ? boardShow = false : null">
    ...
    <div class="content">
      <div v-if="boardSystemBoard.some(it => it.id == 2)"
           :class="{listCont:true, wtA:true , bigWit: !boardSystemBoard.some(it => it.id == 3)}">
        <div>
          <span class="closeBox iconfont icon-shanchu1" @click="delBoardHandle(2)"></span>
          <div class="title"><span>异常设备(今天)</span><span class="mon" @click="() => $router.push('/alarmLog/list')">更多></span>
          </div>
          <div class="table">
            <div class="head">
            ...
            <div class="conttent" style="height: 200px;overflow: scroll;">
              <div v-for="it in tableData" :key="it.name" class="trLine">
              ...
              <div v-if="tableData.length == 0" style="line-height: 200px;text-align: center;color: #999;">
                暂无异常设备报警！
              </div>
            </div>
          </div>
      ...
      <div v-if="boardSystemBoard.some(it => it.id == 3)"
           :class="{listCont:true, wtB:true , bigWit: !boardSystemBoard.some(it => it.id == 2)}">
        <span class="closeBox iconfont icon-shanchu1" @click="delBoardHandle(3)"></span>
        <div class="title">
          <span>异常趋势（单位：个）</span>
          <ChartsFilter :isTypeSelectShow="false" mark='trend' @timeCallback="timeCallback"
                        @typeCallback="typeCallback"/>
        </div>
        <div class="cont">
          <one-line-chart :key="chartRest" :chart-data="trendChartDatas" height="250px" width="100%"/>
        </div>
      </div>
      <div v-if="boardSystemBoard.some(it => it.id == 4)"
           :class="{listCont:true, bigWit: unSystemBoardList.length == 0}">
        <span class="closeBox iconfont icon-shanchu1" @click="delBoardHandle(4)"></span>
        <div class="title">
          <span>异常数量Top10(单位：个）</span>
          <ChartsFilter :isTypeSelectShow="false" mark='top' @timeCallback="timeCallback" @typeCallback="typeCallback"/>
        </div>
        <div class="cont">
          <bar-chart :key="restTop" :chartData="top10Data" height="270px" width="100%"/>
        </div>
      </div>
      <div v-for="(item, ind) in unSystemBoardList" :key="ind"
           :class="{listCont:true, bigWit: unSystemBoardList.length % 2 == 0 && ind+1 == unSystemBoardList.length}">
        <span class="closeBox iconfont icon-shanchu1" @click="delBoardHandle(item.id)"></span>
        <div class="title">
          <span>{{ item.name }}</span>
          <ChartsFilter :isTypeSelectShow="false" :mark="item.id" @timeCallback="timeCallback"
                        @typeCallback="typeCallback"/>
        </div>
        <div v-if="item.chartsData" :key="restKey" class="cont">
          <line-chart :key=restKeyList[ind] :chart-data="item.chartsData" height="240px" width="100%"/>
        </div>
        <div v-if="!item.chartsData" class="cont" style="    text-align: center; line-height: 200px;">
          暂无数据！
        </div>
      </div>
    </div>
    <div class="addDashboard">
      <div class="addBut" @click.stop="() => boardShow = true">编辑看板
        <div v-show="boardShow" class="dishboardSelect">
    ...
    >
      ...
      <el-checkbox-group v-model="checkList" class="checkbox">
        <el-checkbox disabled label="1">设备分布</el-checkbox>
...
export default class extends Vue {
  ...
  // 异常设备实时数据 今天
  getMqttData() {
    var client = mqtt.connect('ws://192.168.171.128/mqtt', {
      port: 8083,
