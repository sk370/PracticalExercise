...
<!-- ![](/images/javase/java-1/b03533fa828ba61edbddc04d4034970a304e59a4) -->
...
![image.png](/images/javase/java-1/image-1660367464182.png)

5. JDK1.5之后支持自动装箱、自动拆箱，但类型必须匹配
   - Integer a = 10;   自动封箱
   - int b = a;        自动拆箱
...
1. 继承自Object
1. 使用双引号包裹。
1. 字符串的字符使用Unicode字符编码，一个字符（不区分字母和汉字）占用2个字节。
1. 常见创建方式（常用构造器）：
   - `new String()`：空参
   - `new String(String str)`：字符串字面量
   - `new String(char[] a)`：字符型数组
   - `new String(byte[] b)`：btye型数字（一个byte数占1个字节）
   - `new String()`：
5. String由final修饰，不可被继承。**源码**`**public final class String{}**`
5. String内定义了`private final char value[]`，用于存放字符串内容。
   - 由于value数组被final修饰，所以不能改变value的地址值。
   - 如果改变字符串变量的字面量值，如将`String str = s;`改为`String str = str;`则表示str指向常量池中新的地址。而不是对常量池中s所在的地址空间重新填充内容。
   - 体现：
...
4. 字面方式创建注意点：
   ...
   - `String a = "hello";String b = "abc";String c = a + b;`与`String e = "hello" + "abc";`不等价。
      - `String c = a + b;`底层会先进行`StringBuilder c = new StringBuilder()`，然后调用append()方法将a和b添加进去，此时创建的StringBuilder()对象在堆内存中，所以c保存的是堆空间的地址。
      - 如果再执行`c.intern();`则与`String e = "hello" + "abc";`等价。
         - 原因为intern()方法会在常量池中查找是否存在该字符串，存在时返回常量池中的对象（含地址），不存在则创建。
   - `String a = "hello";String b = "abc" +  a;`只要有变量参与，就会调用StringBuilder()创建对象。
      - 但假如`final String a = "hello"`，则结果仍在常量池，因为此时a变成了常量
   - 结论：
...
1. jdk 1.7：字符串常量池存储在堆空间
1. jdk 1.8：字符串常量池存储在方法区（元空间）
...
1. 特性：
   ...
   - 作为参数传递时，方法内部可以改变值。
2. 构造器：
   - `StringBuffer()`：初始容量为16的char型字符串数组
   - `StringBuffer(int size)`：指定size容量的字符串
...
1. JDK8之前：
>
> - java.lang.System类：`currentTimeMillis()`，获取当前时间距离1970年1月1日0时0分0秒的毫秒数
> - java.util.Date类  ：
>   - 构造器：
>     - `Date()`
>     - `Date(long date)`
>   - 方法：
>     - `getTime()`：获取当前时间距离1970年1月1日0时0分0秒的毫秒数
>     - `toString()`：【重写方法】按照down mon dd hh:mm:ss zzz yyy的格式输出
>       - down：星期几
>       - zzz：时间标准
>   - java.sql.Date类：
>     - java.util.Date类的子类
...
>       - 这里sourse的格式必须与创建`SimpleDateFormat()`对象的格式一致
>       - 有异常问题
...
>     - Calendar时抽象类，无法实例化，调用静态方法`getInstance()`实际上会调用其子类`GregorianCalendar`创建对象。
>   - 常用方法：
>     - `int get(int field,int value)`
>     - `void set(int field,int value)`
>     - `void add(int field,int amount)`
>     - `Date getTime()`：日历类转换Date类
>     - `void setTime(Date date)`：Date类转换日历类
...
5. JDK8新增时间类（构造方法私有的，不能实例化）：
   ...
   - DateTimeFormatter：格式化或解析日期、时间
      ...
      - 解析：`时间格式.parse(符合时间格式的字符串)`，返回TemporalAccessor类型日期

### 5.4 其他类

1. Math类：
   - 常用方法：
      - `double floor(double a)`：——向坐标轴左侧取整，如1.2、1.6得1.0；-1.2、-1.6得-2.0。
...
4. BigDecimal类
4. Array类：
   - `toString()`：重写Object方法，默认调用，输出数组元素
...
5. 补充：
   ...
   - String具有`compareTo`、`conpareToIngoreCase`方法
6. 比较：
...
1. enumeration、enum
1. 一组常量类型的集合。
1. 是一种特殊的类，只包含有限的特定的对象，与Object并列
1. 特点：
...
1. 特点：和注释一样，不影响程序运行，但注解可以被编译或运行，相当于嵌入代码的补充信息。
1. 功能：
...
4. 源码中`@interface xxx`表示xxx是一个注解类。
4. 元注解：修饰其他注解的注解
...
7. `@WebServle()`：
7. `@Transactional(propagation=Propagation.REQUIRES_NEW, isolation=Isolation.READ_COMMITTED,readOnly=false,timeout=3)`：
...
:::
![image.png](/images/javase/java-1/image-1660367481422.png)
...
1. Error：Java虚拟机无法解决的严重问题。一般不做处理。
...
1. "throws + 异常类型"写在方法的声明处，指明此方法执行时，可能会抛出的异常类型。
   - 抛出多个异常使用`,`分隔
2. 一旦当方法体执行时，出现异常，仍会在异常代码处生成一个异常类的对象，此对象满足throws后异常类型时，就会被抛出。不满足则相当于异常没有处理。异常代码后续的代码，就不再执行！
2. throws的方式只是将异常抛给了方法的调用者。并没有真正将异常处理掉，方法调用者再抛出时，异常类型要不小于接收到的异常类型。
2. 运行异常的默认处理方式为throws，可不处理，编译异常没有默认处理方式，必须处理
2. 子类重写父类的方法时，对抛出异常的规定:子类重写的方法，抛出的异常类型要么和父类抛出的异常一致，要么为父类抛出的异常类型的子类型
2. 在throws 过程中，如果有方法 try-catch , 就相当于处理异常，就可以不必throws

### 7.4 try-catch-finally和throws的选择

1. 拥有继承关系的父子类，如果父类中被重写的方法没有throws方式处理异常，则子类重写的方法也不能使用throws，意味着如果子类重写的方法中有异常，必须使用try-catch-finally方式处理。
1. 执行的方法a中，先后又调用了另外的几个方法，这几个方法是递进关系执行的。建议这几个方法使用throws的方式进行处理。而执行的方法a可以考虑使用try-catch-finally方式进行处理。

### 7.5 手动抛出异常

1. 在可能发生异常的位置生成异常类对象，然后通过throw语句实现抛出操作
1. 可以抛出的异常必须是Throwable或其子类的实例，否则会报错。
...
2. 提供全局常量（static final修饰）：serialVersionUID
2. 提供重载的构造器，传入`String msg`参数
2. 构造器内执行`super(msg);`
